% ================================================================
% gear_fatigue_v3.6.tex — Gear Bending Fatigue Variance Decomposition
...
% All numbers from output/aft_anova.json (EM estimation, 2000 bootstrap) — v3.6
...
ratio is $\sim$0.39---two factors dominate:
...
(e.g., Bonaiti~et~al.\ for standard choice; Dowling~et~al.\ for
...
reported 35.3\% standard-choice variance, and the reported value should
...
dominate: mean-stress correction (39.1\%, S/N~$\approx$~15) and
...
The S--N curve source ($-$0.7\%, S/N~$\approx$~6) and the
Standard~$\times$~$R_z$ interaction (2.5\%, S/N~$\approx$~3) are secondary but
distinguishable from zero. The cumulative damage rule (0.3\%,
...
0.3\% figure must therefore \emph{not} be generalized to
...
Table~\ref{tab:anova} (e.g., 33.7\% vs.\ 39.1\% for mean-stress
...
scatter. This is a single-point comparison---one gear geometry, one material,
one loading condition---and cannot support a general claim that method
uncertainty universally dominates material scatter. It does suggest
that, for this material and at
...
in Table~\ref{tab:anova} supports this
...
0.3\% finding. Their study operates on a single methodological axis
...
recommendations below must therefore be interpreted as applying to
...
version), and the three dominant
  ...
  \item \textbf{Applicability bound at high censoring fractions.} The
        ...
        23.5\%) should therefore be interpreted as the best available
  ...
  \item \textbf{No residual stress; through-hardened steel only.} The
        ...
        therefore represents a nominal baseline; the true mean-stress
