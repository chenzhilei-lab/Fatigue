% ================================================================
% gear_fatigue_v3.1.tex — Gear Bending Fatigue Variance Decomposition
% v3.1: Full rewrite — AFT numbers corrected to EM-estimated ground truth,
...
% All numbers from output/aft_anova.json (EM estimation, 2000 bootstrap) — v3.1
...
\end{tabular}
\medskip\par
\small $^*$ At 500~N$\cdot$m, only 14 of 144 combinations predict
finite life; the AFT decomposition is dominated by the censored entries
and negative or zero variance fractions are numerical artifacts of an
underpowered design. These values are included for completeness but
should not be interpreted as reliable point estimates.
\end{table}
...
\medskip
\noindent\textbf{Quantitative benchmark: method spread vs.\ material scatter.}
To benchmark the method-induced spread against this measured material
scatter, I ran the full 144-combination sweep of my pipeline on Wang's
exact gear geometry at stress levels producing a comparable mix of
finite-life and run-out entries ($\sigma_{F0} \approx 640$--700~MPa).
The method-induced $\log_{10}(N_f)$ spread is 2.1--3.3~dex---a factor
of 2--3$\times$ larger than the $\sim$1~dex material scatter at a
fixed stress. In other words, for the same gear geometry and material,
the choice of standard, mean-stress correction, and roughness
assumption introduces \emph{more} prediction variability than the
intrinsic scatter of the material itself. This is the central
engineering message of the present study: method-induced uncertainty
is not a minor correction to be applied after material characterization;
it is the dominant source of dispersion in gear fatigue life prediction,
and it must be quantified and managed with the same rigor as material
property scatter. The finding is corroborated by an independent gear
geometry and loading condition, strengthening its generality beyond
the reference case of Table~\ref{tab:gear}.
