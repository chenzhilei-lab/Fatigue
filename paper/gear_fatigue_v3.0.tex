...
A Computational Variance Decomposition Across Five Engineering Choices}}
...
differ by five orders of magnitude. The divergence comes not from
...
correct for mean stress. This is a purely numerical study---no
...
mean-stress correction (33.9\%), size-and-surface standard (30.1\%), and
surface roughness (24.6\%). A Standard~$\times$~Rz interaction accounts
for 3.0\%. The S--N curve source (7.5\%) and cumulative damage rule
(0.9\%) are secondary. Bootstrap validation confirms stable
...
$\mathrm{S/N} = \bar{\theta}_j / \sigma_{\rm boot,\,j}$ is the
...
torque range is left for future work.
...
framework is a priority for future work.
...
Size/surface standard & 43.3\% & 32.7\% & 30.1\% \\
...
($\sigma_{F0}\approx 555$~MPa) lies near the material endurance limit
(500--550~MPa). For 130 of 144 combinations, the effective stress after
mean-stress correction and size/surface factors falls below the
endurance limit, producing right-censored run-out observations. With
only 14 uncensored data points to estimate 10 regression coefficients,
the likelihood surface becomes nearly flat in several parameter
directions, and the drop-one-factor log-likelihood differences can
become negative---a numerical artifact of an information-poor design,
not a physically meaningful result. These values are reported for
completeness but should not be interpreted as reliable point estimates.
The 600 and 700~N$\cdot$m levels, with 60 and 116 finite-life entries
respectively, are adequately powered.
\end{table}
...
spans approximately one order of magnitude. To benchmark the
method-induced spread against this measured material scatter, I ran the
full 144-combination sweep of my pipeline on Wang's exact gear geometry
at stress levels producing a comparable mix of finite-life and run-out
entries ($\sigma_{F0} \approx 640$--700~MPa). The method-induced
$\log_{10}(N_f)$ spread is 2.1--3.3~dex---a factor of 2--3$\times$
larger than the $\sim$1~dex material scatter at a fixed stress. In other
words, for the same gear geometry and material, the choice of standard,
mean-stress correction, and roughness assumption introduces \emph{more}
prediction variability than the intrinsic scatter of the material itself.
This independent benchmark, using a different gear geometry and loading
condition, provides quantitative external support for the central claim
of the present study: method-induced uncertainty is not a minor
correction---it is the dominant source of dispersion in gear fatigue
life prediction.
...
campaign is a priority for follow-up work.
...
\begin{enumerate}
  \item \textbf{Stress concentration treatment asymmetry.} The analysis
        ...
        applied to a baseline stress. To assess the sensitivity of the
        standard-choice variance to this modeling choice, I perturbed
        $Y_{Sa}$ uniformly across all three standards over the range
        1.2--1.9, which spans the plausible tooth-root stress
        concentration coefficients for involute spur gears. Over the
        engineering-reasonable sub-range 1.4--1.9, the standard-choice
        variance contribution varies from 27.9\% to 31.6\%---a
        fluctuation of at most $\pm$3 percentage points around the
        baseline 30.1\%. The three dominant factors (mean-stress
        correction, standard, roughness) retain their co-equal ranking
        across the entire range, and their combined variance share
        remains within 84\%--92\%. At $Y_{Sa} \leq 1.3$, the
        decomposition becomes unreliable because the majority of
        entries predict run-out (92--116 censored out of 144). I
        therefore report the standard-choice variance as
        $30.1\% \pm 3\%$, reflecting the sensitivity to the uniform
        $Y_{Sa}$ simplification. The qualitative conclusion---that
        standard choice is a first-order contributor---is robust.% @source: ysa_sensitivity_full.json
  \item \textbf{Scope of the standard-choice comparison.} The present study
        ...
        calculation chains remains a necessary follow-up study.
  ...
  \item \textbf{AFT model assumptions.} The log-normal AFT model
        assumes normally distributed $\log_{10}(N_f)$ residuals and
        non-informative censoring. The Shapiro--Wilk test rejects
        strict normality ($W=0.960$, $p=0.0016$; \S\ref{sec:methods}),
        though the mild skewness ($-0.44$) and the stability of the
        bootstrap variance fractions (Table~\ref{tab:anova}) suggest
        that the practical impact on the variance decomposition is
        limited. Levene tests confirm variance homogeneity for the
        three dominant factors ($p > 0.5$). The censoring pattern is
        physically interpretable (run-outs cluster with Gerber
        correction and ground surfaces; \S\ref{sec:torque}) and is
        unlikely to be informative in the sense of masking
        factor-dependent effects.
...
under constant-amplitude pulsating bending ($R=0$) at 700~N$\cdot$m---a
torque level near the endurance limit for this geometry---across 144
combinations, I find:
...
\begin{enumerate}
  \item At this operating point, three methodological choices are co-equal
        ...
        prediction: mean-stress correction (33.9\%,
        Goodman/Gerber/Morrow/SWT), size-and-surface standard (30.1\%,
        ISO~6336/AGMA~2001/FKM), and surface roughness grade (24.6\%).
        Together they account for 88.6\% of total log-variance. These
        proportions are specific to the high-torque condition studied here;
        the factor ranking shifts with the operating point
        (\S\ref{sec:torque}).
