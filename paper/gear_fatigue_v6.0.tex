...
native stress-chain sensitivity analysis (replacing the unified
...
using AGMA's $J$-factor and a Peterson/Neuber approximation of FKM's
$K_f$ rather than the full FKM guideline, quantifies the bias for this
specific gear geometry. A complete native calculation pipeline---full
FKM $K_f$ workflow without
Peterson/Neuber approximation, and digitized standard-specific stress
gradient procedures---remains open in Phase~2 to eliminate the
...
gradients and support effects is not replicated here) was re-run on
...
The numerical value of $+13.2$~pp is specific to this gear and the
midpoint $J=0.33$: the AGMA $J$ sensitivity spans 64.3\% ($J=0.31$,
conservative) to 72.0\% ($J=0.35$, non-conservative), and using the
Neuber $K_f$ instead of Peterson gives 65.4\%
(\texttt{output/native\_chain.json}). The direction of the change
does, however,
...
\textbf{FKM completeness caveat.} The FKM $K_f$ used here is a
Peterson/Neuber notch-sensitivity approximation, not the full FKM
guideline procedure (stress gradients, support effects, and the
official lookup tables). The full FKM native workflow is not yet
implemented; the FKM column above should therefore be read as
``approximate-FKM'', and the associated dex-scale offsets carry an
unquantified systematic component. This limitation is stated once
here and applies to every FKM-derived number in the paper.
...
\begin{enumerate}
  ...
  \item \textbf{Scope of the standard-choice comparison.} The present study
        ...
        between 64.3\% ($J=0.31$) and 72.0\% ($J=0.35$), with 65.4\%
        under the Neuber $K_f$. In every native scenario the standard
        factor remains dominant; a
        fully native implementation of the complete FKM guideline
        procedure (stress gradients and official lookup tables) remains
        open as Phase~2 of the research programme
