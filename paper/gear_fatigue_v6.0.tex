...
official $K_f$ changes the share by only $+0.2$~pp relative to the
Peterson value
...
gradient $G_\sigma(r) = 2/r$ (tooth-root fillet radius
$r = 1.14$~mm) and the global gradient $G_\sigma(d) = 2/d$
(d = 4.71~mm), and the steel constants of Table~2.3.1
($a_G = 0.90$, $b_G = 1200$~MPa, $R_m = 1000$~MPa);
$K_f = K_t/\left[n_\sigma(r)\,n_\sigma(d)\right]$ (Eq.~2.3.2).
For this gear geometry the official procedure gives
$K_f = 1.851$, within $0.5$\% of the Peterson value
($K_f = 1.843$); re-running the native-chain decomposition with the
official $K_f$ changes the standard share from 68.5\% to 68.7\%
($\Delta = +0.2$~pp, \texttt{output/fkm\_official\_chain.json}).
...
\begin{enumerate}
  ...
  \item \textbf{Scope of the standard-choice comparison.} The present study
        ...
        between 64.3\% ($J=0.31$) and 72.0\% ($J=0.35$), with 68.7\%
