...
\begin{enumerate}
  ...
  \item \textbf{Manufacturing and operating sensitivity.} The
        ...
        (iv)~\emph{Geometry}
        (\texttt{multi\_geometry\_sensitivity.json}): for a 14-tooth
        pinion the ranking reverses (mean-stress 36.8\% vs.\ standard
        32.9\%), because the higher tooth-root stress amplifies
        mean-stress sensitivity; for $m_n=5$ and $8$~mm at 700~N$\cdot$m
        all 144 combinations are run-outs (the stress falls below the
        fatigue strength), so the decomposition is not identified there.
        The single-geometry ranking of the main analysis is therefore
        specific to $m_n=3$, $z_1=20$ at 700~N$\cdot$m.
        ...
        finite-life). The damage rule remains a minor contributor even
        under random loading, but its share triples relative to
        constant-amplitude operation; a full nonlinear
        sequence-effect damage model (beyond the simplified load
        interaction used here) remains future work.
