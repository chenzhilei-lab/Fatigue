...
A gear's fatigue life, this study demonstrates, is not stamped into
the steel during heat treatment. It is written---differently---into
...
Imagine the same gear standing before three tribunals. The evidence is
identical---the same geometry, the same 42CrMo4 steel, the same 700~N$\cdot$m
bending moment. Three judges consult three law books. The first (ISO~6336)
declares the gear immortal---run-out beyond $10^7$ cycles. The second
(FKM) pronounces a death sentence at $10^4$ cycles. The third (AGMA~2001)
delivers a hung verdict somewhere in between. The gear has not changed.
The law books have.

This is not a parable. It is the everyday reality of gear design. Every
industrial gearbox is engineered by a designer who opens a standard,
selects a correction method, and computes a life estimate---unknowingly
entering what we term the \textbf{Standard Gamble}: a lottery in which
the predicted fatigue life of an identical gear can span
2.79~dex (a factor of $\sim$620) purely through legitimate, defensible
methodological choices, none of which violates its respective published
standard. The stakes are hidden, because few organizations run the same
gear through three competing standards to discover the spread. This
paper quantifies that spread, attributes it to individual choices, and
shows that more than half of the total prediction variance---55.3\% (the surface-factor component isolated under a common ISO stress baseline; a lower bound on full standard divergence, \S\ref{sec:stress_concentration})--- % @source: size_surface_standard_var_pct
originates from a single decision: which size-and-surface factor formulation to trust.
...
this section, and the risk classifications
...
confirm the existence of systematic prediction offsets between
...
The most conservative defensible combination---FKM standard, SWT
...
median life of approximately $1.3\times10^3$~cycles, the global
minimum across all 144 full-factorial combinations.
...
FKM~+~SWT generates a predicted life approximately four orders of
magnitude shorter than a team using ISO~+~Gerber for the same gear
($1.3\times10^{3}$~cycles vs.\ run-out beyond $10^7$~cycles).
...
The following numerical control case only illustrates how the UQ
...
decomposition. \emph{Phase~2}---in progress---has four components:
\begin{itemize}
  ...
  \item \textbf{Case-carburized gears.} Introducing carburized
        steel (16MnCr5) adds the surface residual-stress field as an
        explicit factor, quantifying how the compressive residual
        stress offsets the mean-stress correction contribution
        (18.2\% here for the residual-stress-free baseline).
  ...
  \item \textbf{Multi-material, multi-module database.} S--N curve
        batches across additional steels and heat treatments (the
        five-curve sensitivity of \S\ref{sec:method_dominates} being
        the first step) and module sizes $m_n=2$--$4$~mm, feeding a
        cross-standard correction library with explicit
        applicability boundaries.
\end{itemize}
\emph{Phase~3}, a longer-term objective, will synthesize the Phase~2
data into practical engineering tools: (i)~a set of
standard-discrepancy correction coefficients that map the prediction
offset between any pair of standards as a function of torque and
surface finish, and (ii)~an error-compensation formula that allows
designers to estimate the methodological uncertainty band for a given
gear specification without re-running the full factorial sweep. The
...
The three tribunals of the Introduction---ISO, AGMA, and FKM---deliver
their verdicts not because they disagree on the evidence, but because
they disagree on what the evidence means. This study quantifies exactly
how much each disagreement costs.
...
\textbf{The Standard Gamble} is not a theoretical curiosity. Every
industrial gearbox in service was designed under one of these
standards---and therefore under one of these invisible bets. This
framework makes the bet visible, quantifies the stakes, and gives
engineers the first tool for deciding whether their handbook is helping
them or betting against them. A gear's fatigue life is not stamped into
the steel during heat treatment. It is written---differently---into
every handbook on the engineer's shelf. The question is no longer
whether the handbooks disagree. It is what to do about it.
\end{enumerate}
