% ================================================================
% gear_fatigue_v2.9.tex — Gear Bending Fatigue Variance Decomposition
% v2.9: ANOVA→AFT (censored likelihood), all numbers updated:
%       33.9%/30.7%/24.7%/6.7%/1.1%/3.0%; Bootstrap 50 draws
...
correction (33.9\%), size-and-surface standard (30.7\%), and surface
roughness (24.7\%). A Standard~$\times$~Rz interaction accounts for
3.0\%. The S--N curve source (6.7\%) and cumulative damage rule (1.1\%)
...
We fit a log-normal accelerated failure time (AFT) model to
...
We bootstrap-resample the 144 entries (with replacement) 200 times
...
factors robustly above sampling noise., recomputing all variance fractions on each draw. The
...
All formulas verified against ISO~6336-3:2019, AGMA~2001-D04, FKM~7th~ed.
S--N parameters from Waterloo SMDIdbase. Bootstrap: 200 draws, RNG seed~42.}
\label{tab:anova}
\begin{tabular}{lrrr}
\toprule
Factor & $\Delta$logLik & Var.\% & S/N \\
\midrule
Mean stress correction & & & & & 33.9 & >10 \\
Size/surface standard & & & & & 30.7 & >10 \\
Surface roughness, Rz & & & & & 24.7 & >10 \\
S--N curve source & & & & & 6.7 & 4 \\
Standard $\times$ Rz & & & & & 3.0 & --- \\
Cumulative damage rule & & & & & 1.1 & 2 \\
\midrule
\sigma_{\rm AFT} & & & & & 0.225 & --- \\
\bottomrule
\end{tabular}
...
handled through censored likelihood. Three factors share the
dominant contribution: mean-stress correction (33.9\%, S/N~$>$~10),
size-and-surface standard (30.7\%, S/N~$>$~10), and surface roughness
(24.7\%, S/N~$>$~10). Together they account for 89.3\% of total
log-variance. Bootstrap confidence intervals are narrow
($\pm$2--3 percentage points), confirming that all three are robustly
above the noise floor.

The S--N curve source (6.7\%, S/N~=~4) and the
Standard~$\times$~Rz interaction (3.0\%) are secondary but
distinguishable from zero. The cumulative damage rule (1.1\%,
S/N~=~2) is negligible in the constant-amplitude regime. The
AFT scale parameter $\sigma = 0.225$ indicates that the model
...
The S--N curve source (6.7\%) is non-negligible: the two Waterloo
...
higher-endurance S--N curve. The cumulative damage rule (1.1\%) is
...
Bootstrap 95\% confidence intervals (200 draws, RNG seed~42) confirm
that the three-factor ranking is stable under resampling.
Mean-stress correction (mean 33.5\%, CI [33.1, 36.9]),
size-and-surface standard (mean 30.3\%, CI [29.0, 33.8]),
and surface roughness (mean 24.4\%, CI [23.9, 27.1])
all have S/N ratios well above 10, with narrow, non-overlapping
confidence intervals. The S--N curve source (mean 6.6\%, CI [4.7, 8.7])
is clearly distinguishable from zero, while the cumulative damage
rule (mean 1.1\%, CI [0.3, 2.4]) is marginal.
...
\begin{enumerate}
  \item Three methodological choices dominate the variance in gear
        bending fatigue life prediction: mean-stress correction (33.9\%,
        Goodman/Gerber/Morrow/SWT), size-and-surface standard (30.7\%,
        ISO~6336/AGMA~2001/FKM), and surface roughness grade (24.7\%).
        Together they account for 89.3\% of total log-variance.
  \item The AFT censored-likelihood framework reveals that surface
        roughness---previously underestimated at 8.5\% by ordinary
        ANOVA---is a first-order contributor because run-out
        combinations are strongly associated with low-Rz surfaces.
  \item The S--N curve source (6.7\%) is non-negligible, while the
        cumulative damage rule (1.1\%) is minor for constant-amplitude
        loading.
